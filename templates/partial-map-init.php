...
if (isset($regions) && isset($block_attributes['region']) && $block_attributes['region'] != '') {
    ...
    $regions_filtered = array_filter(
        $regions,
        function ($obj) use ($cbn_start_region_name) {
...
foreach ($posts as $post) {
    ...
    $location_meta = isset($indexed_meta[$post_id]['_cbn_location_key']) ?
        maybe_unserialize($indexed_meta[$post_id]['_cbn_location_key']) : [];
    ...
    $image = $indexed_meta[$post_id]['_cbn_location_image'] ?? '';
    $image_thumb = null;
    ...
    $audio = $indexed_meta[$post_id]['_cbn_location_audio'] ?? '';
    ...
    if (is_array($meta_custom_fields) && is_array($active_custom_fields)) {
        ...
        foreach ($active_custom_fields as $index => $custom_field) {
            ...
            $custom_fields[] = [
                ...
                'fieldtype' => $custom_field['fieldtype'] ?? 'text',
                'uselabelastextoption' => $custom_field['uselabelastextoption'] ?? false,
            ];
        }
    }
