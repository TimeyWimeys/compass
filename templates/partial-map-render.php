...
foreach ($locations_list as $location){
  ...
  if(isset($location['custom_fields']) && is_array($location['custom_fields'])) {
    ...
    foreach($location['custom_fields'] as $custom_field) {
        if (empty($custom_field['val'])) continue;
...
<div class="box-wrap map-size-<?php echo esc_attr($map_size); ?> <?php if($oum_enable_regions == 'on' && $regions && count($regions) > 0): ?>oum-regions-<?php echo $oum_regions_layout_style; ?> <?php endif; ?>">
  <?php if($oum_enable_regions == 'on' && $regions && count($regions) > 0): ?>
    <div class="tab-wrap">
      <div class="oum-tabs" id="nav-tab-<?php echo $unique_id; ?>" role="tablist">
        <?php
        $i = 0;
        ?>
        <?php foreach ($regions as $region): ?>
          ...
          ?>
          <div class="nav-item nav-link <?php echo (isset($oum_start_region_name) && $name == $oum_start_region_name) ? 'active' : ''; ?> change_region" data-lat="<?php echo esc_attr($term_lat); ?>" data-lng="<?php echo esc_attr($term_lng); ?>" data-zoom="<?php echo esc_attr($term_zoom); ?>" data-toggle="tab" role="tab"><?php echo esc_html($name); ?></div>
  ...
  <div class="map-wrap">
    ...
    <?php if($oum_enable_searchbar === 'true' && $oum_searchbar_type == 'markers'): ?>
      <div id="oum_search_marker"></div>
    <?php endif; ?>

    <?php if($oum_enable_searchbar === 'true' && $oum_searchbar_type == 'live_filter'): ?>
      <input type="text" id="oum_filter_markers" class="oum-hidden" placeholder="<?php echo esc_attr($oum_searchmarkers_label); ?>" />
    <?php endif; ?>
        ...
        <?php if ( oum_fs()->can_use_premium_code() ): ?>
          ...
          <?php if(get_option('oum_enable_user_restriction')): ?>
            ...
            <?php if(is_user_logged_in()): ?>
              ...
              <div id="open-add-location-overlay" class="open-add-location-overlay oum-hidden" style="background-color: <?php echo $oum_ui_color; ?>"><span class="btn_icon">+</span><span class="btn_text"><?php echo esc_attr($oum_plus_button_label); ?></span></div>

            <?php elseif(!is_user_logged_in() && get_option('oum_enable_redirect_to_registration')): ?>

              <a href="<?php echo wp_registration_url(); ?>" class="open-add-location-overlay oum-hidden" style="background-color: <?php echo $oum_ui_color; ?>"><span class="btn_icon">+</span><span class="btn_text"><?php echo esc_attr($oum_plus_button_label); ?></span></a>
    ...
    <?php if($types): ?>
      <div class="oum-filter-controls <?php echo $oum_collapse_filter; ?> oum-hidden">
    ...
    <?php if ( oum_fs()->is__premium_only() ): ?>
      <?php if ( oum_fs()->can_use_premium_code() ): ?>
        ...
        <?php if($oum_disable_oum_attribution != 'on'): ?>
          ...
          <div class="oum-attribution">made with <a href="https://www.open-user-map.com/?ref=map" title="Open User Map | Everybody can add locations" target="_blank">OUM PRO</a></div>
    ...
    <script type="text/javascript" data-category="functional" class="cmplz-native" id="oum-inline-js">
      ...
      if(document.getElementById(map_el)) {
        ...
        var oum_all_locations = <?php echo json_encode($oum_all_locations, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES); ?>;
        ...
        var oum_enable_searchbar = <?php echo $oum_enable_searchbar; ?>;
        var oum_searchbar_type = `<?php echo $oum_searchbar_type; ?>`;
        ...
        var oum_enable_searchaddress_button = <?php echo $oum_enable_searchaddress_button; ?>;
        var oum_searchaddress_label = `<?php echo esc_attr($oum_searchaddress_label); ?>`;

        var oum_enable_searchmarkers_button = <?php echo $oum_enable_searchmarkers_button; ?>;
        var oum_searchmarkers_label = `<?php echo esc_attr($oum_searchmarkers_label); ?>`;
        var oum_searchmarkers_zoom = `<?php echo esc_attr($oum_searchmarkers_zoom); ?>`;
        ...
        var oum_use_settings_start_location = <?php echo $oum_use_settings_start_location; ?>;
        var oum_has_regions = <?php echo ($oum_enable_regions == 'on' && $regions && count($regions) > 0)? 'true' : 'false'; ?>;
        var oum_enable_multiple_marker_types = `<?php echo $oum_enable_multiple_marker_types; ?>`;

        var oum_location = {};
        var oum_custom_css = '';
        var oum_custom_script = '';
        var oum_max_image_filesize = <?php echo esc_attr($oum_max_image_filesize); ?>;
        var oumMap;
        var oumMap2;
        ...
        <?php if($oum_ui_color): ?>
          ...
          oum_custom_css += `
            ...
            .open-user-map .box-wrap .map-wrap .oum-filter-controls .oum-filter-list .close-filter-list:hover {color: <?php echo $oum_ui_color; ?> !important}
            ...
            .open-user-map .oum_location_text a {color: <?php echo $oum_ui_color; ?> !important}
            ...
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .media-upload label {color: <?php echo $oum_ui_color; ?> !important}
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .oum-image-upload .media-upload-top label .multi-upload-indicator {background: <?php echo $oum_ui_color; ?> !important}
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .oum-video-upload input[type=text]:hover {border-color: <?php echo $oum_ui_color; ?> !important}
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .oum-video-upload input[type=text]:focus {border-color: <?php echo $oum_ui_color; ?> !important; box-shadow: 0 0 0 2px <?php echo $oum_ui_color; ?>1a !important}
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .image-preview-placeholder {border-color: <?php echo $oum_ui_color; ?> !important; background: <?php echo $oum_ui_color; ?>0a !important}
            .open-user-map .add-location .location-overlay-content #oum_add_location .oum_media .oum-image-preview-grid .image-preview-item.dragging {border-color: <?php echo $oum_ui_color; ?> !important}`;
        ...
        <?php if($oum_map_height): ?>
          ...
          /* custom map height */
          oum_custom_css += `
            .open-user-map .box-wrap > .map-wrap {padding: 0 !important; height: <?php echo esc_attr($oum_map_height); ?> !important; aspect-ratio: unset !important;}`;
        ...
        <?php if($oum_map_height_mobile): ?>
          ...
          /* custom map height */
          oum_custom_css += `
            @media screen and (max-width: 768px) {.open-user-map .box-wrap > .map-wrap {padding: 0 !important; height: <?php echo esc_attr($oum_map_height_mobile); ?> !important; aspect-ratio: unset !important;}}`;
