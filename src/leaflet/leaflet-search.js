				...
				_recordsFromAjax: function (text, callAfter) {
					...
					if (window.XMLHttpRequest === undefined) {
						window.XMLHttpRequest = function () {
							try {
								return new ActiveXObject( "Microsoft.XMLHTTP.6.0" ); } catch (e1) {
								try {
									return new ActiveXObject( "Microsoft.XMLHTTP.3.0" ); } catch (e2) {
		...
		L.Control.Search.Marker = L.Marker.extend(
			{
				...
				animate: function () {
					...
					if (this._circleLoc) {
						...
						circle._timerAnimLoc = setInterval(
							function () {
								acc    += 0.5;
