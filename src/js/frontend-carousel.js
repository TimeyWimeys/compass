...
const CBNCarousel = (function () {
	...
	function initializeCarousel() {
		...
		document.querySelectorAll( '.Compass-locations-list .cbn-carousel' ).forEach(
			carousel => {
            setupCarousel( carousel );
			}
		);
		...
		document.querySelectorAll( '.cbn-location-value .cbn-carousel' ).forEach(
			carousel => {
            setupCarousel( carousel );
			}
		);
	...
	function setupCarouselNavigation(items, prevBtn, nextBtn, counter, currentIndex) {
		...
		document.addEventListener(
			"keydown",
			(e) => {
            if ( ! items[currentIndex].closest( ".leaflet-popup-content-wrapper" )) {
                return;
            }
            if (e.key === "ArrowLeft") {
		...
		// Add touch support
		let touchStartX     = 0;
		items[0].parentElement.addEventListener(
			"touchstart",
			(e) => {
