...
/* global jQuery, ajaxurl, wp, GeoSearch, L, console */
/**
...
let CBNFormController = (function () {
	...
	function showFormMessage(type, headline, message, buttonText = null, buttonCallback = null) {
		...
		if (finalHeadlineEl) {
			finalHeadlineEl.textContent = headline || '';
		}
		if (finalMessageEl) {
	...
	function setupCloseEvents() {
		...
		// Backdrop click
		document.getElementById( "add-location-overlay" ).addEventListener(
			"click",
			function (event) {
	...
	function setupNotificationEvents() {
		...
		if (notificationCheckbox) {
			notificationCheckbox.addEventListener(
				"change",
				function () {
	...
	function populateForm(location) {
		...
		if (titleField) {
			titleField.value = location.title || "";
		}
		if (latField) {
		...
		if (location.custom_fields && Array.isArray( location.custom_fields )) {
			location.custom_fields.forEach(
				field => {
					if ( ! field || typeof field.index === 'undefined') {
						return;
					}
...
let CBNMedia = (function () {
	...
	function updatePlaceholderPosition(e) {
		...
		let closestSibling = siblings.reduce(
			(closest, child) => {
